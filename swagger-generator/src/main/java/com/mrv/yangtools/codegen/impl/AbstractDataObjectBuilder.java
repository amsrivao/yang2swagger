...
public abstract class AbstractDataObjectBuilder implements DataObjectBuilder {
    ...
    @Override
    public String addModelForComplexTypes(TypeDefinition enumType) {
        QName qName = enumType.getQName();
        ...
        if(! generatedEnums.containsKey(qName)) {
            log.debug("generating enum model for {}",  qName);
            String name = getName(qName);
            if(enumType instanceof BitsTypeDefinition) {
                BitsTypeDefinition bitType = (BitsTypeDefinition) enumType;
                ModelImpl enumModel = buildBits(bitType, qName);
                swagger.addDefinition(name, enumModel);
                generatedEnums.put(qName, DEF_PREFIX + name);
            }
            if(enumType instanceof UnionTypeDefinition) {
            	UnionTypeDefinition bitType = (UnionTypeDefinition) enumType;
                ModelImpl enumModel = buildUnion(bitType, qName);
                swagger.addDefinition(name, enumModel);
                generatedEnums.put(qName, DEF_PREFIX + name);
            }
    ...
    protected ModelImpl buildBits(BitsTypeDefinition enumType, QName qName) {
        ModelImpl model = new ModelImpl();
        List<String> stringList = new ArrayList<String>();
        for(Bit bit : enumType.getBits()) {
            stringList.add(bit.getName());
        }
        //model.setEnum(enumType.getBits().stream().collect(Collectors.toList());
        model.setEnum(stringList);
        model.setType("string");
        model.setReference(getName(qName));
        return model;
    }
    
    protected ModelImpl buildUnion(UnionTypeDefinition unionType, QName qName) {
        ModelImpl model = new ModelImpl();
        List<String> stringList = new ArrayList<String>();
        stringList.add(unionType.getUnits());
